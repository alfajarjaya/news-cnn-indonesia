...
const Internasional = () => {
    ...
    const { data: nasionalNews, loading: nasionalLoading, error: nasionalError } = DATA_API('internasional');
    const { data: teknoNews, loading: teknoLoading, error: teknoError } = DATA_API('teknologi');
    const { data: ekoNews, loading: ekoLoading, error: ekoError } = DATA_API('ekonomi');
    ...
    const paginatedNews = nasionalNews.slice(startIdx, endIdx);
    ...
    return (
        <>
            ...
            <div className="mt-15">
                ...
                <h2 className="text-center mb-4 fs-5 text-gray">Top international news update for today !</h2>
            </div>
            <div className="container mb-5 d-flex justify-content-between">
                <div className="row mx-3 w-100 h-100">
                    <div className='d-flex align-items-center mb-5'>
                        <h3 className='fw-bold mx-2'>INTERNASIONAL <span className='text-danger'>|</span></h3>
                        <h6>
                            {['teknologi', 'ekonomi'].map((page) => (
                                    ...
                                    <ul className="pagination">
                                        {currentPage > 1 && (
                                            <li className="page-item">
                                                <a className="page-link" href={`/id/internasional?page=${currentPage - 1}`} aria-label="Previous">
                                        ...
                                        {[...Array(10)].map((_, index) => {
                                            ...
                                            return (
                                                <li key={page} className={`page-item ${page === currentPage ? 'active' : ''}`}>
                                                    <a className="page-link" href={`/id/internasional?page=${page}`}>{page}</a>
                                                </li>
                                            );
                                        })}
                                        ...
                                        {currentPage < 10 && (
                                            <li className="page-item">
                                                <a className="page-link" href={`/id/internasional?page=${currentPage + 1}`} aria-label="Next">
                    ...
                    {[...news, ...ekoNews, ...teknoNews].sort(() => Math.random() - 0.5).slice(0, 10).map((article, index) => (
