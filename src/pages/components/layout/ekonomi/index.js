...
const Ekonomi = () => {
    ...
    // Fetch API data
    const { data: news, loading, error } = DATA_API('internasional');
    ...
    const paginatedNews = ekoNews.slice(startIdx, endIdx);
    ...
    return (
        <>
            ...
            <div className="mt-15">
                ...
                <h2 className="text-center mb-4 fs-5 text-gray">Economy news update for today !</h2>
            </div>
            <div className="container mb-5 d-flex justify-content-between">
                <div className="row mx-3 w-100 h-100">
                    <div className='d-flex align-items-center mb-5'>
                        <h3 className='fw-bold mx-2'>ECONOMY <span className='text-danger'>|</span></h3>
                        <h6>
                            {['nasional', 'internasional'].map((page) => (
                                    ...
                                    <ul className="pagination">
                                        {currentPage > 1 && (
                                            <li className="page-item">
                                                <a className="page-link" href={`/id/ekonomi?page=${currentPage - 1}`} aria-label="Previous">
                                        ...
                                        {[...Array(10)].map((_, index) => {
                                            ...
                                            return (
                                                <li key={page} className={`page-item ${page === currentPage ? 'active' : ''}`}>
                                                    <a className="page-link" href={`/id/ekonomi?page=${page}`}>{page}</a>
                                                </li>
                                            );
                                        })}
                                        ...
                                        {currentPage < 10 && (
                                            <li className="page-item">
                                                <a className="page-link" href={`/id/ekonomi?page=${currentPage + 1}`} aria-label="Next">
                    ...
                    {[...news, ...nasionalNews, ...teknoNews].sort(() => Math.random() - 0.5).slice(0, 10).map((article, index) => (
                ...
                <div className="d-none d-md-block row d-flex flex-column justify-content-end align-items-end" style={{ width: '50%' }}>
                    ...
                    {nasionalNews.sort(() => Math.random() - 0.5).slice(0, 9).map((article, index) => (
