...
const Nasional = () => {
    ...
    const isLoading = [loading, nasionalLoading, lifeLoading, funLoading].some((status) => status);
    if (isLoading) return <div className="text-center mt-10">Loading...</div>;

    // Memeriksa apakah ada error
    const hasError = [error, nasionalError, lifeError, funError].find((err) => err);
    ...
    return (
        <>
            ...
            <div className="mt-15">
                ...
                <h2 className="text-center mb-4 fs-5 text-gray">Top national news update for today !</h2>
            </div>
            <div className="container mb-5 d-flex justify-content-between">
                <div className="row mx-3 w-100 h-100">
                    <div className='d-flex align-items-center mb-5'>
                        <h3 className='fw-bold mx-2'>NASIONAL <span className='text-danger'>|</span></h3>
                        <h6>
                            {['ekonomi', 'olahraga'].map((page) => (
                                    ...
                                    <ul className="pagination">
                                        {currentPage > 1 && (
                                            <li className="page-item">
                                                <a className="page-link" href={`/id/nasional?page=${currentPage - 1}`} aria-label="Previous">
                                        ...
                                        {[...Array(10)].map((_, index) => {
                                            ...
                                            return (
                                                <li key={page} className={`page-item ${page === currentPage ? 'active' : ''}`}>
                                                    <a className="page-link" href={`/id/nasional?page=${page}`}>{page}</a>
                                                </li>
                                            );
                                        })}
                                        ...
                                        {currentPage < 10 && (
                                            <li className="page-item">
                                                <a className="page-link" href={`/id/nasional?page=${currentPage + 1}`} aria-label="Next">
                    ...
                    {[...news, ...lifeNews, ...funNews].sort(() => Math.random() - 0.5).slice(0, 9).map((article, index) => (
